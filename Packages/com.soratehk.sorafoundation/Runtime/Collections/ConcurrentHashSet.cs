...
namespace SoraTehk.Collections {
    ...
    public class ConcurrentHashSet<T> : IReadOnlyCollection<T>, ICollection<T> {
        ...
        public int Count {
            get {
                ...
                try {
                    ...
                    for (var i = 0; i < countPerLocks.Length; i++) count += countPerLocks[i];
                }
                finally {
        ...
        public bool IsEmpty {
            get {
                if (!AreAllBucketsEmpty()) return false;
        ...
        void ICollection<T>.CopyTo(T[] array, int arrayIndex) {
            ...
            try {
                ...
                for (var i = 0; i < countPerLock.Length && count >= 0; i++) count += countPerLock[i];
